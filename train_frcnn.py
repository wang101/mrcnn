...
model_classifier.compile(optimizer=optimizer_classifier, loss=[losses.class_loss_cls, losses.class_loss_regr(len(classes_count)-1), losses.class_loss_seg()], metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
...
for epoch_num in range(num_epochs):
    ...
    while True:
        ...
        if True:
            ...
            X, Mask, Y, img_data = next(data_gen_train)
            ...
            X2, Y1, Y2, YM, IouS = roi_helpers.calc_iou(R, img_data, C, class_mapping)
            ...
            loss_class = model_classifier.train_on_batch([X, X2[:, sel_samples, :]], [Y1[:, sel_samples, :], Y2[:, sel_samples, :], YM[:, sel_samples, :, :]])
