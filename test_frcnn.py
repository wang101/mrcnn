...
def make_mask(seg, coords, imgshape):
    assert imgshape[-1] == 3
...
for idx, img_name in enumerate(sorted(os.listdir(img_path))):
    ...
    for jk in range(R.shape[0]//C.num_rois + 1):
        ...
        [P_cls, P_regr, P_seg] = model_classifier_only.predict([F, ROIs])
        P_pred = np.squeeze(np.argmax(P_seg,axis=-1))

        classes_array = [np.argmax(P_cls[0,i,:]) for i in range(P_cls.shape[1])]
        SKIP = True
        proposals = range(P_cls.shape[1])
        if 0 not in classes_array or np.all(P_cls[0,:,0]<bbox_threshold):
            SKIP = False
            proposals = [np.argmax(P_cls[0,:,0]),]
        ...
        for ii in proposals:
            ...
            segment[cls_name].append(P_pred[ii])
    ...
    for key in bboxes:
        ...
        new_boxes, new_probs,pick = roi_helpers.non_max_suppression_fast(bbox, np.array(probs[key]), overlap_thresh=0.5)
        new_segment = np.array(segment[key])[pick]
        for jk in range(new_boxes.shape[0]):
            (x1, y1, x2, y2) = new_boxes[jk,:]
            new_segment_jk = new_segment[jk]
